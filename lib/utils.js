...
/**
 * Convert the relative path url test to the absolute path /test
 * Determine if the url does not start with /, then splice a /
 * At the same time, if there are multiple repeated // at the beginning, only one will be retained.
 * @param {*} str
 */
export function convertUrlStartWithOneSlash(str) {
  ...
  // Determine whether the url starts with /
  if (!str.startsWith("/")) {
    // If not, splice a / in front
    str = "/" + str;
  }
