...
export async function getPostBlocks(id, from, slice) {
  const cacheKey = "page_block_" + id;
  // console.log("getPostBlocks:", cacheKey);
  let pageBlock = await getDataFromCache(cacheKey);
  if (pageBlock) {
...
function filterPostBlocks(id, pageBlock, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (b?.value?.type === "sync_block" && b?.value?.children) {
      ...
      // Remove sync block
      delete clonePageBlock.block[blockId];
      // Replace synchronized blocks with subblocks
      ...
      // Reprocess newly added sub-blocks
      i--;
      continue;
    }
