...
export async function getCustomedGlobalData({
  ...
  // Get from notice
  const data = await getAllNotionPageData({ pageId, from });
...
export async function getGlobalData({
  ...
  const data = await getAllNotionPageData({ pageId, from, type });
  const db = deepClone(data);
  // Sensitive data not returned
  delete db.block;
  delete db.schema;
 ...
 */
export async function getAllNotionPageData({ type = "Record", pageId, from }) {
  // Try to get from Cache
  // console.log("getAllNotionPageData: pageId:", pageId);
  const cacheKey = "page_block_" + pageId;
  let data = await getDataFromCache(cacheKey);
  if (data && data.pageIds?.length > 0) {
    return data;
  }
  data = await getAllNotionDataBaseByNotionAPI({ pageId, from, type });
  // cache
  if (data) {
...
function isCollectionView(rawMetadata) {
  if (
...
export async function getPageByPageIdAndType(paramId, recordType) {
