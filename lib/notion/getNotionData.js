...
export async function getGlobalData({
  ...
  // Get from notice
  console.log(`getGlobalData pageId:${pageId}, from:${from}, type:${type}`);
 ...
 * Get announcements
 */
async function getGuestBook(post) {
  if (!post) {
    return null;
  }

  post.blockMap = await getPostBlocks(post.id, "data-guestbook");
  return post;
}
...
async function getDataBaseInfoByNotionAPI({ pageId, from, type = "Record" }) {
  ...
  const introPage = await getGuestBook(
    collectionData.filter((post) => {
      return (
        post &&
        post?.type &&
