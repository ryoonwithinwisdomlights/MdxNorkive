...
export async function getGlobalData({
  ...
  // Get from notice
  // console.log(` pageId:${pageId}, from:${from}, type:${type}`);
...
async function getDataBaseInfoByNotionAPI({ pageId, from, type = "Post" }) {
  console.log("[Fetching Data]", pageId, from, type);
  ...
  const allPages = collectionData.filter((post) => {
    ...
    if (type === "Writing-records") {
      if (post?.type === "Writing-records" && post.status === "Published") {
        postCount++;
      }
    ...
    } else if (type === "General-records") {
      if (post?.type === "General-records" && post.status === "Published") {
        postCount++;
      }
    ...
    } else if (type === "Engineering-records") {
      if (post?.type === "Engineering-records" && post.status === "Published") {
        postCount++;
      }
    } else {
      postCount++;
