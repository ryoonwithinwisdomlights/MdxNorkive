...
/**
 * PDF 링크를 PdfWrapper 컴포넌트로 변환
 * [파일명.pdf](URL) → <PdfWrapper names={"파일명"} urls={"URL"} />
 */
function transformPdfLinks(content) {
  ...
  return content.replace(
    ...
    (match, fileName, url) => {
      ...
      return `<PdfWrapper names={"${nameWithoutExtension}"} urls={"${url}"} />`;
    }
  );
}
...
function transformYouTubeLinks(content) {
  ...
  return content.replace(
    /\[([^\]]+)\]\((https?:\/\/(www\.)?(youtube\.com|youtu\.be|youtube-nocookie\.com)\/[^\s)]+)\)/g,
    (match, linkText, url) => {
      // URL에서 쿼리 파라미터 제거 (si= 등)
      const cleanUrl = url.split("?")[0];
      return `<YoutubeWrapper names={"${linkText}"} urls={"${cleanUrl}"} />`;
    }
  );
}
